
#include <iostream>
#include <time.h>


using namespace std;

typedef unsigned char byte; // 새로운 데이터 타입 : byte = GF(2^8)


byte sbox[256] = {
, };

byte isbox[256] = {
, };


void AES8_SubBytes(byte in[16], byte out[16]) {
	for (int i = 0; i < 16; i++)
		out[i] = sbox[in[i]];
}
/*
0	4	8	12
1	5	9	13
2	6	10	14
3	7	11	15
*/
void AES8_ShiftRows(byte in[16], byte out[16]) {
	out[0] = in[0];
	out[4] = in[4];
	out[8] = in[8];
	out[12] = in[12];

	out[1] = in[5];
	out[5] = in[9];
	out[9] = in[13];
	out[13] = in[1];

	out[2] = in[10];
	out[6] = in[14];
	out[10] = in[2];
	out[14] = in[6];

	out[3] = in[15];
	out[7] = in[3];
	out[11] = in[7];
	out[15] = in[11];
}

//GF(2^8) xtime()함수
byte GF_Xtime(byte gf) {//Xtime
	return ((gf >> 7) & 0x01 ? (gf << 1) ^ 0x1b : gf << 1);//x^8이 밀려나가는데 x^8 = x^4 + x^3 + x^2 + x^0 = 0x1b
}


/*
2	3	1	1 ]  [ 0  4  8 12 ]
1	2	3	1 ]  [ 1  5  9 13 ]
1	1	2	3 ]  [ 2  6 10 14 ]
3	1	1	2 ]  [ 3  7 11 15 ]
반복 사용하지 않고 이렇게 구하는게 훨씬 빠르다
xtime은 비트연산만 하기 때문에 상당히 빠름 mul은 느리다
*/
void AES8_MixColumns(byte in[16], byte out[16]) {//  2 * a = a + a = GF_Xtime(a),  3 * a = a ^ a ^ a = GF_Xtime(a) ^ a //주의 여기서 3도 갈로아필드로 보고 x+1로 생각해야한다.
	out[0] = GF_Xtime(in[0]) ^ GF_Xtime(in[1]) ^ in[1] ^ in[2] ^ in[3]; //2 3 1 1  
	out[1] = in[0] ^ GF_Xtime(in[1]) ^ GF_Xtime(in[2]) ^ in[2] ^ in[3]; //1 2 3 1
	out[2] = in[0] ^ in[1] ^ GF_Xtime(in[2]) ^ GF_Xtime(in[3]) ^ in[3]; //1 1 2 3
	out[3] = GF_Xtime(in[0]) ^ in[0] ^ in[1] ^ in[2] ^ GF_Xtime(in[3]); //3 1 1 2

	out[4] = GF_Xtime(in[4]) ^ GF_Xtime(in[5]) ^ in[5] ^ in[6] ^ in[7]; //2 3 1 1
	out[5] = in[4] ^ GF_Xtime(in[5]) ^ GF_Xtime(in[6]) ^ in[6] ^ in[7]; //1 2 3 1
	out[6] = in[4] ^ in[5] ^ GF_Xtime(in[6]) ^ GF_Xtime(in[7]) ^ in[7]; //1 1 2 3 
	out[7] = GF_Xtime(in[4]) ^ in[4] ^ in[5] ^ in[6] ^ GF_Xtime(in[7]); //3 1 1 2 

	out[8] = GF_Xtime(in[8]) ^ GF_Xtime(in[9]) ^ in[9] ^ in[10] ^ in[11];  //2 3 1 1 
	out[9] = in[8] ^ GF_Xtime(in[9]) ^ GF_Xtime(in[10]) ^ in[10] ^ in[11]; //1 2 3 1 
	out[10] = in[8] ^ in[9] ^ GF_Xtime(in[10]) ^ GF_Xtime(in[11]) ^ in[11];//1 1 2 3
	out[11] = GF_Xtime(in[8]) ^ in[8] ^ in[9] ^ in[10] ^ GF_Xtime(in[11]); //3 1 1 2

	out[12] = GF_Xtime(in[12]) ^ GF_Xtime(in[13]) ^ in[13] ^ in[14] ^ in[15]; //2 3 1 1 
	out[13] = in[12] ^ GF_Xtime(in[13]) ^ GF_Xtime(in[14]) ^ in[14] ^ in[15]; //1 2 3 1
	out[14] = in[12] ^ in[13] ^ GF_Xtime(in[14]) ^ GF_Xtime(in[15]) ^ in[15]; //1 1 2 3
	out[15] = GF_Xtime(in[12]) ^ in[12] ^ in[13] ^ in[14] ^ GF_Xtime(in[15]); //3 1 1 2
}

void AES8_AddRoundKey(byte in[16], byte roundkey[16], byte out[16]) {
	for (int i = 0; i < 16; i++) {
		out[i] = in[i] ^ roundkey[i];//라운드키의 길이가 1byte. 즉 8bit임을 유의
	}
}

void AES8_Round(byte in[16], byte rk[16], byte out[16]) {
	byte state1[16], state2[16];

	AES8_SubBytes(in, state1);
	AES8_ShiftRows(state1, state2);
	AES8_MixColumns(state2, state1);
	AES8_AddRoundKey(state1, rk, state2);

}

void AES8_Encrypt(byte PT[16], byte RK[11][16], byte CT[16]) {
	byte state1[16], state2[16];

	AES8_AddRoundKey(PT, RK[0], state1);

	for (int i = 1; i < 10; i++) {//9번이지만 1부터 9까지 이므로 범위를 이렇게 해줘용
		AES8_Round(state1, RK[i], state2);
		for (int j = 0; j < 16; j++)//state1에 state2를 복사해야함
			state1[j] = state2[j];
	}

	AES8_SubBytes(state1, state2);
	AES8_ShiftRows(state2, state1);
	AES8_AddRoundKey(state1, RK[10], CT);
}

//암호화 중간값 출력
void AES_Print_State(byte state[16], const char* msg = nullptr) {//만약 재정의가 되지 않으면 nullptr값을 주라는 뜻, const가 없으면 에러뜸('pt'가 상수이므로) 상수는 변수에 넣지 않는 값 따라서 변하지 않는다. 리터럴은 메모리를 차지하지 않고 메모리 풀에 있는 값을 이용 msg라는 리터럴 풀의 주소를 가진다
	if (msg != nullptr)
		printf("%s = ", msg);
	for (int i = 0; i < 16; i++)
		printf("%02x ", state[i]);
	printf("\n");
}
